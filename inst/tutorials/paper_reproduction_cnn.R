...
# Quick mode: reduced set (E ∈ {1,5,20}, B=∞ only)
# Configs: 1 (E=1), 2 (E=5), 4 (E=20) - all with B=∞
...
# Time estimate (B=∞ is faster than B=10,50)
time_per_exp <- if (Sys.getenv("FEDAVGR_QUICK", "0") == "1") 15 else 20 # minutes per experiment
total_time <- nrow(experiments) * 2 * time_per_exp
cat(sprintf(
    "Estimated time: %.1f hours (%.0f min/experiment)\n\n",
    total_time / 60, time_per_exp
))
...
for (i in 1:nrow(experiments)) {
    ...
    for (partition_type in c("IID", "nonIID")) {
        ...
        if (!is.null(result)) {
            ...
            append_metrics(result$history, "metrics_mnist.csv")
...
table2_final$B <- ifelse(is.infinite(table2_final$B), "∞", as.character(table2_final$B))
...
plot_data <- combined_results %>%
    mutate(
        B_label = ifelse(is.infinite(as.numeric(B)), "B=∞", paste0("B=", B)),
        E_label = paste0("E=", E),
        B_num = ifelse(is.infinite(as.numeric(B)), 999, as.numeric(B))
    ) %>%
    arrange(B_num, E)
...
cat("  - Raw data: metrics_mnist.csv\n\n")
